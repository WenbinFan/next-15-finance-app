...
export default function Page() {
  ...
  return <>
    ...
    <form className="space-y-4" action={formAction}>
      ...
      {!state?.error && state?.message.length > 0 && <Alert icon={<Check className="text-green-700 dark:text-green-300 w-6 h-6"/>} title={<span className="text-green-700 dark:green-red-300">Success</span>}><span className="text-green-700 dark:text-green-300">{state?.message}</span></Alert>}
